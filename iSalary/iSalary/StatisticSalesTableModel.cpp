...
StatisticSalesTableModel::StatisticSalesTableModel(QObject* parent){
    this->id = StatisticSalesTableModel::maxId++;
}
...
QVariant StatisticSalesTableModel::headerData( int section, Qt::Orientation orientation, int role ) const {
	...
	case COLUMN_NAME:
        return QString::fromWCharArray( L"Товар" );
	case COLUMN_PRICE:
        return QString::fromWCharArray( L"Стоимость, руб." );
	case COLUMN_COMMISION:
        return QString::fromWCharArray( L"Комиссия, %" );
    }
