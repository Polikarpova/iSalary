...
void Product_DB::createTable() {
    ...
    query.prepare( "CREATE TABLE  IF NOT EXISTS `" + TABLE_NAME +
                   "` (`id` int NOT NULL PRIMARY KEY AUTO_INCREMENT, `name` NVARCHAR(45) NOT NULL,`commission` DOUBLE NOT NULL, `isDeleted` INT(1) NOT NULL "
                   "DEFAULT '0')" );
...
void Product_DB::update( Product product ) {
    ...
    QSqlQuery query( *db );
    query.prepare( QString( "UPDATE " + TABLE_NAME + " SET `name` = :name, `commission` = :commission, `isDeleted` = :isDeleted WHERE id = " ) +
                   QString::number( product.getId() ) );
    query.bindValue( ":name", product.getName() );
    query.bindValue( ":commission", product.getCommission() );
