...
EmployeesTableModel::EmployeesTableModel( QObject *parent ) { this->id = EmployeesTableModel::maxId++; }
...
QVariant EmployeesTableModel::data( const QModelIndex &index, int role ) const {
    if ( !index.isValid() || managerTable.count() <= index.row() || ( role != Qt::DisplayRole && role != Qt::EditRole ) ) {
        return QVariant();
    }
