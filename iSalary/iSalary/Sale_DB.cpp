...
void Sale_DB::createTable() {
    ...
    query.prepare( "CREATE TABLE  IF NOT EXISTS `" + TABLE_NAME +
                   "` (`id` int NOT NULL PRIMARY KEY AUTO_INCREMENT, `manager_id` int NOT NULL, `product_id` int NOT NULL, `productName` varchar(45) CHARACTER "
                   "SET utf8 NOT NULL, `productCommission` double NOT NULL, `price` DOUBLE NOT NULL, `count` int NOT NULL, `isActive` INT(1) NOT NULL, "
                   "`isConfirmed` INT(1) NOT NULL, `saleDate` date NOT NULL DEFAULT \"1000-01-01\", `confirmDate` DATE NOT NULL DEFAULT \"1000-01-01\" ) "
...
void Sale_DB::create( ActiveSale sale ) {
    ...
    QString sql = "INSERT INTO " + TABLE_NAME +
                  " (`manager_id`, `product_id`, `productName`, `productCommission`, `price`, `count`, `isActive`, `isConfirmed`, `saleDate`, `confirmDate`) "
                  "VALUES(:manager_id, :product_id, :name, :productCommission, :price, :count, :isActive, :isConfirmed, :saleDate, :confirmDate)";
    query.prepare( sql );
...
QList<ManagerActiveSalesStatisticDTO> Sale_DB::getManagerActiveSalesSatistic() {
    ...
    while ( query.next() ) {
        ...
        QString sql2 = "select users.id, users.firstName, users.secondName, users.thirdName, sales.isConfirmed from users, sales where sales.manager_id = "
                       "users.id AND sales.isActive = 1 AND users.id = :id";
...
QList<ActiveSaleDTO> Sale_DB::getActiveSales() {
    ...
    QString sql =
        "select users.firstName, users.secondName, users.thirdName, sales.* from sales, users where sales.manager_id = users.id AND sales.isActive = 1";
    QSqlQuery query( *db );
    query.prepare( sql );
...
QList<ActiveSaleDTO> Sale_DB::getActiveSalesForManager( int managerId ) {
    ...
    //создаем строку с sql-запросом
    QString sql = "select users.firstName, users.secondName, users.thirdName, sales.* from sales, users where sales.manager_id = users.id AND sales.isActive = "
                  "1 AND users.id = :id";
    QSqlQuery query( *db );
    query.prepare( sql );
...
ManagerActiveSalesStatisticDTO Sale_DB::readToDTO( const QSqlQuery &query, const QSqlQuery &queryWithManagerData ) {
    ...
    result.managerName = queryWithManagerData.value( "secondName" ).value<QString>() + " " + queryWithManagerData.value( "firstName" ).value<QString>() + " " +
...
ManagerActiveSalesStatisticDTO Sale_DB::readEmptyResultToDTO( const QSqlQuery &query ) {
    ...
    result.managerName =
        query.value( "secondName" ).value<QString>() + " " + query.value( "firstName" ).value<QString>() + " " + query.value( "thirdName" ).value<QString>();
    result.confirmCount = 0;
    result.unconfirmCount = 0;
...
QList<SaleInfoDTO> Sale_DB::getSalesInfoConfimedFromPeriod( int id, QDate from, QDate to ) {
    ...
    QString sql = "select sales.price, sales.count, sales.productCommission from sales, users where sales.manager_id = users.id AND sales.isConfirmed = 1 AND "
                  "users.id = :id AND sales.confirmDate >= :dateFrom AND sales.confirmDate <= :dateTo;";
    QSqlQuery query( *db );
    query.prepare( sql );
...
QList<ActiveSaleDTO> Sale_DB::getConfirmedSales( int id, QDate from, QDate to ) {
    ...
    QString sql = "select users.firstName, users.secondName, users.thirdName, sales.* from sales, users where sales.manager_id = users.id AND "
                  "sales.isConfirmed = 1 AND users.id = :id AND sales.confirmDate >= :dateFrom AND sales.confirmDate <= :dateTo;";
    QSqlQuery query( *db );
    query.prepare( sql );
    query.bindValue( ":id", id );
    query.bindValue( ":dateFrom", from );
...
QList<ActiveSaleDTO> Sale_DB::getConfirmedSalesSoldFromPeriod( int id, QDate from, QDate to ) {
    ...
    QString sql = "select users.firstName, users.secondName, users.thirdName, sales.* from sales, users where sales.manager_id = users.id AND "
                  "sales.isConfirmed = 1 AND users.id = :id AND sales.saleDate >= :dateFrom AND sales.saleDate <= :dateTo;";
    QSqlQuery query( *db );
    query.prepare( sql );
    query.bindValue( ":id", id );
    query.bindValue( ":dateFrom", from );
